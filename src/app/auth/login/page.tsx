...
import { Eye, EyeOff, Mail, Lock, ArrowRight, Sparkles, Phone } from 'lucide-react';
...
export default function ProviderLoginPage() {
    ...
    const handleSubmit = async (e: React.FormEvent) => {
        ...
        try {
            ...
            const onboardingStep = response.data?.onboarding_step;
            ...
            if (accessToken) {
                ...
                } else if (onboardingStep && onboardingStep < 4) {
                    router.push(`/auth/register?step=${onboardingStep + 1}`);
                } else {
                    router.push('/provider/dashboard');
                }
            } else {
                showToast("Login failed: Unauthorized", "error");
            }
        } catch (error: any) {
