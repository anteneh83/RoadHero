...
export default function ReviewsPage() {
    ...
    const handleAction = async (review: Review) => {
        const replyMessage = prompt(`Enter your reply for ${review.author}:`);
        if (!replyMessage) return;

        try {
            await reviewService.reply(review.id, {
                reply_message: replyMessage,
                mark_resolved: review.urgent
            });
            window.dispatchEvent(new CustomEvent('show-toast', {
        ...
        <div className="space-y-8 animate-in fade-in slide-in-from-bottom-4 duration-1000 pb-20">
            ...
            <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-6">
                <div>
                    ...
                    <p className="text-[10px] font-bold text-gray-400 dark:text-gray-500 uppercase tracking-widest mt-1">{t('reviews_subtitle')}</p>
                </div>
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
                <div className="bg-white/70 dark:bg-white/5 backdrop-blur-md p-8 rounded-[32px] border border-white/40 dark:border-white/5 shadow-xl shadow-primary/5 space-y-4 hover:shadow-2xl transition-all duration-700 group relative overflow-hidden text-left">
                    <p className="text-[10px] font-black text-gray-400 dark:text-gray-500 uppercase tracking-[0.2em] leading-none transition-colors">{t('overall_reputation')}</p>
                    {isLoading ? <Loader2 className="animate-spin text-accent" /> : (
                        <div className="flex items-end gap-3 relative z-10 transition-transform duration-500 group-hover:translate-x-1">
                            <h2 className="text-5xl font-black text-[#171717] dark:text-white tracking-tighter transition-colors">{stats?.overall_rating || '0.0'}</h2>
                    ...
                    <p className="text-[10px] font-bold text-gray-400 dark:text-gray-500 uppercase tracking-widest transition-colors">{t('based_on_reviews').replace('{count}', String(stats?.total_reviews || 0))}</p>
                </div>

                <div className="bg-white/70 dark:bg-white/5 backdrop-blur-md p-8 rounded-[32px] border border-white/40 dark:border-white/5 shadow-xl shadow-primary/5 space-y-6 hover:shadow-2xl transition-all duration-700 group relative text-left">
                    ...
                    <div className="space-y-3">
                        <div className="flex items-baseline justify-between">
                            <h2 className="text-4xl font-black text-green-500 tracking-tighter group-hover:scale-105 transition-transform origin-left">{stats?.response_rate_percentage || 0}%</h2>
                        ...
                        <div className="w-full h-2 bg-gray-50 dark:bg-white/5 rounded-full overflow-hidden shadow-inner border border-gray-100 dark:border-white/5 transition-colors">
                            <div
                                className="h-full bg-gradient-to-r from-green-400 to-green-600 dark:from-green-500 dark:to-green-700 rounded-full shadow-[0_0_12px_rgba(34,197,94,0.3)] transition-all duration-1000"
                ...
                <div className="bg-white/70 dark:bg-white/5 backdrop-blur-md p-8 rounded-[32px] border border-white/40 dark:border-white/5 shadow-xl shadow-primary/5 relative overflow-hidden flex flex-col justify-between hover:shadow-2xl transition-all duration-700 group text-left">
                    ...
                    <div className="relative z-10 transition-colors">
                        ...
                        <h2 className="text-5xl font-black text-primary dark:text-accent mt-4 tracking-tighter group-hover:translate-x-1 transition-transform transition-colors">{stats?.total_reviews || 0}</h2>
                    </div>
                ...
                <div className="space-y-6">
                    {isLoading ? (
                        ...
                        <div className="py-20 text-center">
                            <p className="text-[10px] font-black uppercase tracking-[0.2em] text-gray-400">No reviews found matching your search</p>
                        </div>
                            ...
                            <div className="flex flex-col md:flex-row md:items-start justify-between gap-10">
                                <div className="space-y-6 flex-1">
                                    ...
                                    {review.reply_message && (
                                        <div className="pl-6 border-l-2 border-primary/20 dark:border-accent/20 mt-4">
                                            <p className="text-[10px] font-black uppercase tracking-widest text-primary dark:text-accent mb-1">Your Reply:</p>
                                            <p className="text-sm font-medium italic text-gray-500 dark:text-gray-400">{review.reply_message}</p>
                                        </div>
                                    )}
                                </div>
                                ...
                                <div className="flex flex-col items-end gap-3">
                                    ...
                                    <button
                                        ...
                                        className={cn(
                                            ...
                                            review.reply_message && "opacity-50"
                                        )}>
                                        {review.urgent ? <AlertCircle size={16} /> : <Reply size={16} />}
                                        {review.reply_message ? "Update Reply" : (review.urgent ? t('resolve_issue') : t('reply_to_review'))}
                                    </button>
                                </div>
                            </div>
                        </div>
