    ...
    const handleSubmit = async () => {
        ...
        if (isNaN(numAmount) || numAmount <= 0) {
            window.dispatchEvent(new CustomEvent('show-toast', {
                detail: { message: "Please enter a valid amount", type: 'error' }
            }));
            return;
        }
        ...
        try {
            await revenueService.addWalkIn({
                amount: numAmount,
                payment_method: paymentMethod,
                service_provided: serviceProvided || "General Service",
            });
            window.dispatchEvent(new CustomEvent('show-toast', {
                detail: { message: "Income logged successfully", type: 'success' }
            }));
            onClose(true);
        } catch (error: any) {
            console.error("[IncomeModal] Error:", error);
            window.dispatchEvent(new CustomEvent('show-toast', {
                detail: { message: "Failed to log income", type: 'error' }
            }));
        } finally {
        ...
        <div className="fixed inset-0 z-[100] flex items-center justify-center p-4">
            ...
            <div className="bg-white dark:bg-[#111111] w-full max-w-md rounded-[32px] shadow-2xl relative z-10 p-8 animate-in zoom-in-95 slide-in-from-bottom-8 duration-500 border border-transparent dark:border-white/5 transition-all">
                <button
                    ...
                    className="absolute right-6 top-6 p-1.5 text-gray-400 dark:text-gray-600 hover:text-gray-900 dark:hover:text-white transition-colors disabled:opacity-50"
                >
                    <X size={20} />
                </button>
                ...
                <div className="space-y-8">
                    ...
                    {/* Notes Field */}
                    <div className="space-y-3">
                        <label className="text-[10px] font-black text-gray-400 dark:text-gray-500 uppercase tracking-widest ml-1 transition-colors">Service Description (Optional)</label>
                        <textarea
                            ...
                            disabled={isSubmitting}
                            className="w-full bg-white dark:bg-white/[0.02] border border-gray-100 dark:border-white/5 rounded-[24px] px-5 py-4 text-xs font-medium text-gray-900 dark:text-white placeholder:text-gray-300 dark:placeholder:text-gray-700 outline-none focus:ring-2 focus:ring-primary/10 dark:focus:ring-primary/30 transition-all resize-none shadow-sm disabled:opacity-50"
                        />
                    </div>
                    ...
                    <button
                        ...
                        disabled={isSubmitting}
                        className="w-full h-14 bg-[#F97316] dark:bg-accent rounded-2xl text-white text-[11px] font-black uppercase tracking-[0.2em] shadow-xl shadow-orange-900/10 dark:shadow-none hover:bg-orange-600 dark:hover:bg-accent/80 active:scale-95 transition-all flex items-center justify-center gap-3 disabled:opacity-50 disabled:cursor-not-allowed"
                    >
                        {isSubmitting ? (
                            <>
